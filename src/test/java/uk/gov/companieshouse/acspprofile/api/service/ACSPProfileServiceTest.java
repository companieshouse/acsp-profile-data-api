...
class ACSPProfileServiceTest {
    ...
    @InjectMocks
    private ACSPProfileService service;
    ...
    @Mock
    private ACSPProfileListAggregate ACSPProfileListAggregate;
    @Mock
    ...
    void findExistingFilingHistoryDocumentShouldReturnDocument() {
        ...
        // when
        final Optional<ACSPProfileDocument> actualDocument = service.findExistingFilingHistory(TRANSACTION_ID,
                COMPANY_NUMBER);
    ...
    void findCompanyFilingHistoryListShouldCallRepositoryWithCorrectCategories(List<String> actualCategories,
        ...
        when(repository.findCompanyFilingHistory(any(), anyInt(), anyInt(), any()))
                .thenReturn(ACSPProfileListAggregate);
        when(ACSPProfileListAggregate.getTotalCount()).thenReturn(1);

        // when
        final Optional<ACSPProfileListAggregate> actualFilingHistoryListAggregate = service
    ...
    void findCompanyFilingHistoryListShouldCallRepositoryAndReturnEmptyWhenTotalCountZero() {
        ...
        when(repository.findCompanyFilingHistory(any(), anyInt(), anyInt(), any()))
                .thenReturn(ACSPProfileListAggregate);
        when(ACSPProfileListAggregate.getTotalCount()).thenReturn(0);
    ...
    void findFilingHistoryByEntityIdShouldReturnDocument() {
        ...
        // when
        final Optional<ACSPProfileDeleteAggregate> actualDocument = service.findFilingHistoryByEntityId(TRANSACTION_ID);
