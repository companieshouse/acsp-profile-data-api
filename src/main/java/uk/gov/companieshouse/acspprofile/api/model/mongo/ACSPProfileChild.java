package uk.gov.companieshouse.acspprofile.api.model.mongo;

import com.fasterxml.jackson.annotation.JsonProperty;
import java.util.Objects;
import org.springframework.data.mongodb.core.mapping.Field;

public abstract class ACSPProfileChild {

    @Field("_entity_id")
    @JsonProperty("_entity_id")
    protected String entityId;
    @Field("delta_at")
    @JsonProperty("delta_at")
    protected String deltaAt;

    public String getEntityId() {
        return entityId;
    }

    public abstract ACSPProfileChild entityId(String entityId);

    public String getDeltaAt() {
        return deltaAt;
    }

    public abstract ACSPProfileChild deltaAt(String deltaAt);

    @Override
    public boolean equals(Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        ACSPProfileChild that = (ACSPProfileChild) o;
        return Objects.equals(entityId, that.entityId) && Objects.equals(deltaAt, that.deltaAt);
    }

    @Override
    public int hashCode() {
        return Objects.hash(entityId, deltaAt);
    }

    @Override
    public String toString() {
        return "FilingHistoryChild{" +
                "entityId='" + entityId + '\'' +
                ", deltaAt='" + deltaAt + '\'' +
                '}';
    }
}
